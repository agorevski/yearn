...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const appBaseUrl = process.env.VITE_APP_BASE_URL || 'https://yearn.azurewebsites.net'
    const ogBaseUrl = 'https://og.yearn.fi'
    const ogImageUrl = `${ogBaseUrl}/api/og/yearn/vault/${chainId}/${address}`
    const canonicalUrl = `${appBaseUrl}/vaults/${chainId}/${address}`

    const title = 'Yearn Fork - Vault'
    const description = "Earn yield on your crypto with low fee automated vault strategies"
